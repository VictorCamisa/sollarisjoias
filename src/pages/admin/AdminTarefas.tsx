  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-serif font-semibold">Tarefas</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif font-semibold">Tarefas</h1>
          <p className="text-xs text-muted-foreground mt-1">Organize seu dia a dia</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                <Label>Título</Label>
                <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="rounded-xl" placeholder="O que precisa fazer?" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label>Prazo</Label>
                  <Input type="datetime-local" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} className="rounded-xl" />
                </div>
                <div>
              ...
              <Button onClick={() => createTask.mutate()} disabled={!form.title} className="w-full rounded-xl">
                Criar tarefa
              </Button>
            </div>
          </DialogContent>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "A fazer", count: todoCount, icon: ListTodo, color: "text-muted-foreground" },
          { label: "Em andamento", count: inProgressCount, icon: Clock, color: "text-blue-500" },
          { label: "Concluídas", count: doneCount, icon: CheckCircle2, color: "text-emerald-500" },
        ].map((s, i) => (
      ...
      {/* Task list */}
      {!filtered?.length ? (
        <p className="text-sm text-muted-foreground text-center py-10">Nenhuma tarefa encontrada. 🎉</p>
      ) : (
          ...
          {filtered.map((task) => {
            ...
            return (
              <motion.div key={task.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className={`border border-border rounded-xl p-4 bg-card flex items-start gap-3 ${isDone ? "opacity-60" : ""}`}>
                <Checkbox
                  checked={isDone}
